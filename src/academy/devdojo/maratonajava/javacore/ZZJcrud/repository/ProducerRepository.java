...
public class ProducerRepository {
    public static List<Producer> findByName(String name) {
        ...
        try (Connection conn = ConnectionFactory.getConnection();
             PreparedStatement ps = creatPreparedStatementFindByName(conn, name);
    ...
    private static PreparedStatement creatPreparedStatementFindByName(Connection conn, String name) throws SQLException {
    ...
    public static void delete(int id) {
        try (Connection conn = ConnectionFactory.getConnection();
             PreparedStatement ps = creatPreparedStatementDelete(conn,id)) {
            ps.execute();
    ...
    private static PreparedStatement creatPreparedStatementDelete(Connection conn, Integer id) throws SQLException {
        String sql = "DELETE FROM `anime_store`.`producer` WHERE (`id` = ?);".formatted(id);
        PreparedStatement ps = conn.prepareStatement(sql);
        ps.setInt(1, id);
        return ps;
    }
